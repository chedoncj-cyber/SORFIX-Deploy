"""
Bourse des Valeurs Mobilieres de Tunis (BVMT) FIX 4.2 adapter.

Exchange details:
  Trading hours: 09:00–14:15 CET (UTC+1)
  Settlement:    T+3
  Currency:      TND (Tunisian Dinar)
  FIX gateway:   fix.bvmt.com.tn:9887
  Taker fee:     15 bps
"""
import logging
import threading
...
logger = logging.getLogger("bvmt_adapter")

BVMT_CONFIG = VenueAdapterConfig(
    name="BVMT",
    host="fix.bvmt.com.tn",
    port=9887,
    sender_comp_id="BROKER_TN",
    target_comp_id="BVMT_TRADING",
    currency="TND",
    taker_fee=0.0015,
    latency_ms=5.0,
    enabled=True,
...
def bvmt_tick_size(price: float) -> float:
...
def _round_to_tick(price: float) -> float:
    tick = bvmt_tick_size(price)
    return round(round(price / tick) * tick, 6)


class BVMTAdapter(BaseExchangeAdapter):
    def __init__(self, config: VenueAdapterConfig = None):
        super().__init__(config or BVMT_CONFIG)
    ...
    def connect(self) -> bool:
        logger.info("BVMT: Connecting to %s:%s", self.config.host, self.config.port)
        self._session.logged_on = True
        self._connected = True
        self._hb_stop.clear()
        self._hb_thread = threading.Thread(
            target=self._heartbeat_loop, name="hb-bvmt", daemon=True
        )
        self._hb_thread.start()
        logger.info("BVMT: Session established")
        return True
    ...
    def _heartbeat_loop(self):
        ...
        while not self._hb_stop.wait(timeout=interval):
            try:
                ...
                logger.warning("BVMT: Heartbeat error: %s", exc)

    def subscribe_market_data(self, symbol: str) -> bool:
        if not self._connected:
            logger.warning("BVMT: Cannot subscribe %s — not connected", symbol)
            return False
        ...
        logger.info("BVMT: Subscribed market data for %s", symbol)
        return True
    ...
    def send_order(
        ...
        if not self._connected:
            logger.warning("BVMT: Cannot send order — not connected")
            return False
