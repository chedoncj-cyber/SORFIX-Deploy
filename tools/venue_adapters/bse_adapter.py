"""
Botswana Stock Exchange (BSE) FIX 4.2 adapter.

Exchange details:
  Trading hours: 07:00–14:00 CAT (UTC+2)
  Settlement:    T+3
  Currency:      BWP (Botswana Pula)
  FIX gateway:   fix.bse.co.bw:9883
  Taker fee:     20 bps
"""
import logging
import threading
...
logger = logging.getLogger("bse_adapter")

BSE_CONFIG = VenueAdapterConfig(
    name="BSE",
    host="fix.bse.co.bw",
    port=9883,
    sender_comp_id="BROKER_BW",
    target_comp_id="BSE_TRADING",
    currency="BWP",
    taker_fee=0.0020,
    latency_ms=12.0,
    enabled=True,
...
def bse_tick_size(price: float) -> float:
...
def _round_to_tick(price: float) -> float:
    tick = bse_tick_size(price)
    return round(round(price / tick) * tick, 6)


class BSEAdapter(BaseExchangeAdapter):
    def __init__(self, config: VenueAdapterConfig = None):
        super().__init__(config or BSE_CONFIG)
    ...
    def connect(self) -> bool:
        logger.info("BSE: Connecting to %s:%s", self.config.host, self.config.port)
        self._session.logged_on = True
        self._connected = True
        self._hb_stop.clear()
        self._hb_thread = threading.Thread(
            target=self._heartbeat_loop, name="hb-bse", daemon=True
        )
    ...
    def _heartbeat_loop(self):
        ...
        while not self._hb_stop.wait(timeout=interval):
            try:
                ...
                logger.warning("BSE: Heartbeat error: %s", exc)

    def subscribe_market_data(self, symbol: str) -> bool:
        if not self._connected:
            logger.warning("BSE: Cannot subscribe %s — not connected", symbol)
            return False
        ...
        logger.info("BSE: Subscribed market data for %s", symbol)
        return True
    ...
    def send_order(
        ...
        if not self._connected:
            logger.warning("BSE: Cannot send order — not connected")
            return False
