"""
Casablanca Stock Exchange (CSE) FIX 4.2 adapter.

Exchange details:
  Trading hours: 09:00–14:00 WET (UTC+1)
  Settlement:    T+3
  Currency:      MAD (Moroccan Dirham)
  FIX gateway:   fix.casablanca-bourse.com:9880
  Taker fee:     10 bps
"""
import logging
import threading
...
logger = logging.getLogger("cse_adapter")

CSE_CONFIG = VenueAdapterConfig(
    name="CSE",
    host="fix.casablanca-bourse.com",
    port=9880,
    sender_comp_id="BROKER_MA",
    target_comp_id="CSE_TRADING",
    currency="MAD",
    taker_fee=0.0010,
    latency_ms=4.0,
    enabled=True,
...
def cse_tick_size(price: float) -> float:
...
def _round_to_tick(price: float) -> float:
    tick = cse_tick_size(price)
    return round(round(price / tick) * tick, 6)


class CSEAdapter(BaseExchangeAdapter):
    def __init__(self, config: VenueAdapterConfig = None):
        super().__init__(config or CSE_CONFIG)
    ...
    def connect(self) -> bool:
        logger.info("CSE: Connecting to %s:%s", self.config.host, self.config.port)
        self._session.logged_on = True
        self._connected = True
        self._hb_stop.clear()
        self._hb_thread = threading.Thread(
            target=self._heartbeat_loop, name="hb-cse", daemon=True
        )
        self._hb_thread.start()
        logger.info("CSE: Session established")
        return True
    ...
    def _heartbeat_loop(self):
        ...
        while not self._hb_stop.wait(timeout=interval):
            try:
                ...
                logger.warning("CSE: Heartbeat error: %s", exc)

    def subscribe_market_data(self, symbol: str) -> bool:
        if not self._connected:
            logger.warning("CSE: Cannot subscribe %s — not connected", symbol)
            return False
        ...
        logger.info("CSE: Subscribed market data for %s", symbol)
        return True
    ...
    def send_order(
        ...
        if not self._connected:
            logger.warning("CSE: Cannot send order — not connected")
            return False
