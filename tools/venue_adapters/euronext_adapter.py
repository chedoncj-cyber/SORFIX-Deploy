"""
Euronext (Pan-European) FIX 4.2 adapter.

Exchange details:
  Trading hours: 09:00–17:30 CET (UTC+1/+2) = 07:00–15:30 UTC
  Settlement:    T+2
  Currency:      EUR (Euro)
  FIX gateway:   fix.euronext.com:9888
  Taker fee:     10 bps
"""
import logging
import threading
...
logger = logging.getLogger("euronext_adapter")

EURONEXT_CONFIG = VenueAdapterConfig(
    name="EURONEXT",
    host="fix.euronext.com",
    port=9888,
    sender_comp_id="BROKER_EU",
    target_comp_id="EURONEXT_TRADING",
    currency="EUR",
    taker_fee=0.0010,
    latency_ms=4.0,
    enabled=True,
...
# EUR prices similar to GBP: 0.10–800 EUR
_TICK_TABLE = [
    (0.10,        0.0001),
    (1.00,        0.001),
    (10.00,       0.01),
...
def euronext_tick_size(price: float) -> float:
...
def _round_to_tick(price: float) -> float:
    tick = euronext_tick_size(price)
    return round(round(price / tick) * tick, 6)


class EURONEXTAdapter(BaseExchangeAdapter):
    def __init__(self, config: VenueAdapterConfig = None):
        super().__init__(config or EURONEXT_CONFIG)
    ...
    def connect(self) -> bool:
        logger.info("EURONEXT: Connecting to %s:%s (TLS 1.3)", self.config.host, self.config.port)
        self._session.logged_on = True
        self._connected = True
        self._hb_stop.clear()
        self._hb_thread = threading.Thread(
            target=self._heartbeat_loop, name="hb-euronext", daemon=True
        )
        self._hb_thread.start()
        logger.info("EURONEXT: Session established")
        return True
    ...
    def disconnect(self):
        ...
        self._connected = False
        logger.info("EURONEXT: Disconnected")
    ...
    def _heartbeat_loop(self):
        ...
        while not self._hb_stop.wait(timeout=interval):
            try:
                ...
                logger.warning("EURONEXT: Heartbeat error: %s", exc)

    def subscribe_market_data(self, symbol: str) -> bool:
        if not self._connected:
            logger.warning("EURONEXT: Cannot subscribe %s — not connected", symbol)
            return False
        ...
        logger.info("EURONEXT: Subscribed market data for %s", symbol)
        return True

    def send_order(self, order_id: str, symbol: str, side: str, quantity: float, price: Optional[float]) -> bool:
        if not self._connected:
            logger.warning("EURONEXT: Cannot send order — not connected")
            return False
        ...
        logger.info("EURONEXT: Order [%s] %s %dx%s @ %s EUR", order_id, side.upper(), int(quantity), symbol, "MKT" if price is None else price)
        return True
    ...
    def get_status(self) -> dict:
        return {
            "venue": "EURONEXT", "connected": self._connected,
