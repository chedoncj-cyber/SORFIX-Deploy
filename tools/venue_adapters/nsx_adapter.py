"""
Namibia Stock Exchange (NSX) FIX 4.2 adapter.

Exchange details:
  Trading hours: 07:00–15:00 WAT (UTC+2)
  Settlement:    T+3
  Currency:      NAD (Namibian Dollar)
  FIX gateway:   fix.nsx.com.na:9884
  Taker fee:     20 bps
"""
import logging
import threading
...
logger = logging.getLogger("nsx_adapter")

NSX_CONFIG = VenueAdapterConfig(
    name="NSX",
    host="fix.nsx.com.na",
    port=9884,
    sender_comp_id="BROKER_NA",
    target_comp_id="NSX_TRADING",
    currency="NAD",
    taker_fee=0.0020,
    latency_ms=5.0,
    enabled=True,
...
# NAD ~= ZAR; prices typically 1–100
_TICK_TABLE = [
    (0.50,        0.001),
    (5.00,        0.01),
    (50.00,       0.05),
...
def nsx_tick_size(price: float) -> float:
...
def _round_to_tick(price: float) -> float:
    tick = nsx_tick_size(price)
    return round(round(price / tick) * tick, 6)


class NSXAdapter(BaseExchangeAdapter):
    def __init__(self, config: VenueAdapterConfig = None):
        super().__init__(config or NSX_CONFIG)
    ...
    def connect(self) -> bool:
        logger.info("NSX: Connecting to %s:%s", self.config.host, self.config.port)
        self._session.logged_on = True
        self._connected = True
        self._hb_stop.clear()
        self._hb_thread = threading.Thread(
            target=self._heartbeat_loop, name="hb-nsx", daemon=True
        )
        self._hb_thread.start()
        logger.info("NSX: Session established")
        return True
    ...
    def _heartbeat_loop(self):
        ...
        while not self._hb_stop.wait(timeout=interval):
            try:
                ...
                logger.warning("NSX: Heartbeat error: %s", exc)

    def subscribe_market_data(self, symbol: str) -> bool:
        if not self._connected:
            logger.warning("NSX: Cannot subscribe %s — not connected", symbol)
            return False
        ...
        logger.info("NSX: Subscribed market data for %s", symbol)
        return True
    ...
    def send_order(
        ...
        if not self._connected:
            logger.warning("NSX: Cannot send order — not connected")
            return False
