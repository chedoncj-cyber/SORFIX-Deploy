...
logger = logging.getLogger("szse_adapter")

SZSE_CONFIG = VenueAdapterConfig(
    name="SZSE",
    host="fix.szse.cn",
    port=9885,
    sender_comp_id="BROKER_CN_SZ",
    target_comp_id="SZSE_TRADING",
    currency="CNY",
    taker_fee=0.0003,
    latency_ms=6.0,
    enabled=True,
...
# CNY ChiNext / SME prices typically 1–500 CNY
_TICK_TABLE = [
    (1.00,        0.01),
    (10.00,       0.05),
    (100.0,       0.10),
...
def szse_tick_size(price: float) -> float:
...
def _round_to_tick(price: float) -> float:
    tick = szse_tick_size(price)
    return round(round(price / tick) * tick, 6)


class SZSEAdapter(BaseExchangeAdapter):
    def __init__(self, config: VenueAdapterConfig = None):
        super().__init__(config or SZSE_CONFIG)
    ...
    def connect(self) -> bool:
        logger.info("SZSE: Connecting to %s:%s (TLS 1.3)", self.config.host, self.config.port)
        self._session.logged_on = True
        self._connected = True
        self._hb_stop.clear()
        self._hb_thread = threading.Thread(
            target=self._heartbeat_loop, name="hb-szse", daemon=True
        )
        self._hb_thread.start()
        logger.info("SZSE: Session established")
        return True
    ...
    def _heartbeat_loop(self):
        ...
        while not self._hb_stop.wait(timeout=interval):
            try:
                ...
                logger.warning("SZSE: Heartbeat error: %s", exc)

    def subscribe_market_data(self, symbol: str) -> bool:
        if not self._connected:
            logger.warning("SZSE: Cannot subscribe %s — not connected", symbol)
            return False
        ...
        logger.info("SZSE: Subscribed market data for %s", symbol)
        return True

    def send_order(self, order_id: str, symbol: str, side: str, quantity: float, price: Optional[float]) -> bool:
        if not self._connected:
            logger.warning("SZSE: Cannot send order — not connected")
            return False
        ...
        logger.info("SZSE: Order [%s] %s %dx%s @ %s CNY", order_id, side.upper(), int(quantity), symbol, "MKT" if price is None else price)
        return True
    ...
    def get_status(self) -> dict:
        return {
            "venue": "SZSE", "connected": self._connected,
