"""
Stock Exchange of Mauritius (SEM) FIX 4.2 adapter.

Exchange details:
  Trading hours: 09:30–13:30 MUT (UTC+4)
  Settlement:    T+3
  Currency:      MUR (Mauritian Rupee)
  FIX gateway:   fix.stockexchangeofmauritius.com:9885
  Taker fee:     15 bps
"""
import logging
import threading
...
logger = logging.getLogger("sem_adapter")

SEM_CONFIG = VenueAdapterConfig(
    name="SEM",
    host="fix.stockexchangeofmauritius.com",
    port=9885,
    sender_comp_id="BROKER_MU",
    target_comp_id="SEM_TRADING",
    currency="MUR",
    taker_fee=0.0015,
    latency_ms=8.0,
    enabled=True,
...
def sem_tick_size(price: float) -> float:
...
def _round_to_tick(price: float) -> float:
    tick = sem_tick_size(price)
    return round(round(price / tick) * tick, 6)


class SEMAdapter(BaseExchangeAdapter):
    def __init__(self, config: VenueAdapterConfig = None):
        super().__init__(config or SEM_CONFIG)
    ...
    def connect(self) -> bool:
        logger.info("SEM: Connecting to %s:%s", self.config.host, self.config.port)
        self._session.logged_on = True
        self._connected = True
        self._hb_stop.clear()
        self._hb_thread = threading.Thread(
            target=self._heartbeat_loop, name="hb-sem", daemon=True
        )
        self._hb_thread.start()
        logger.info("SEM: Session established")
        return True
    ...
    def _heartbeat_loop(self):
        ...
        while not self._hb_stop.wait(timeout=interval):
            try:
                ...
                logger.warning("SEM: Heartbeat error: %s", exc)

    def subscribe_market_data(self, symbol: str) -> bool:
        if not self._connected:
            logger.warning("SEM: Cannot subscribe %s — not connected", symbol)
            return False
        ...
        logger.info("SEM: Subscribed market data for %s", symbol)
        return True
    ...
    def send_order(
        ...
        if not self._connected:
            logger.warning("SEM: Cannot send order — not connected")
            return False
