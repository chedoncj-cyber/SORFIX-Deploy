"""
Johannesburg Stock Exchange (JSE) FIX 4.2 adapter — primary South African venue.

Exchange details:
  Trading hours: 07:00–15:00 SAST (UTC+2)
  Settlement:    T+3
  Currency:      ZAR (South African Rand)
  FIX gateway:   fix.jse.co.za:9877 (TLS 1.3 required in production)
  Taker fee:     20 bps
...
logger = logging.getLogger("jse_adapter")

JSE_CONFIG = VenueAdapterConfig(
    name="JSE",
    host="fix.jse.co.za",
    port=9877,
    sender_comp_id="BROKER_ZA",
    target_comp_id="JSE_TRADING",
    currency="ZAR",
    taker_fee=0.002,
    latency_ms=2.0,
    enabled=True,
...
# JSE minimum tick sizes by price band (ZAR)
_TICK_TABLE = [
    (1.00,         0.001),
    (100.0,        0.01),
    (1000.0,       0.10),
...
def jse_tick_size(price: float) -> float:
...
def _round_to_tick(price: float) -> float:
    tick = jse_tick_size(price)
    return round(round(price / tick) * tick, 6)


class JSEAdapter(BaseExchangeAdapter):
    def __init__(self, config: VenueAdapterConfig = None):
        super().__init__(config or JSE_CONFIG)
    ...
    def connect(self) -> bool:
        logger.info("JSE: Connecting to %s:%s (TLS 1.3)", self.config.host, self.config.port)
        self._session.logged_on = True
        self._connected = True
        self._hb_stop.clear()
        self._hb_thread = threading.Thread(
            target=self._heartbeat_loop, name="hb-jse", daemon=True
        )
        self._hb_thread.start()
        logger.info("JSE: Session established")
        return True
    ...
    def _heartbeat_loop(self):
        ...
        while not self._hb_stop.wait(timeout=interval):
            try:
                ...
                logger.warning("JSE: Heartbeat error: %s", exc)

    def subscribe_market_data(self, symbol: str) -> bool:
        if not self._connected:
            logger.warning("JSE: Cannot subscribe %s — not connected", symbol)
            return False
        ...
        logger.info("JSE: Subscribed market data for %s", symbol)
        return True
    ...
    def send_order(
        ...
        if not self._connected:
            logger.warning("JSE: Cannot send order — not connected")
            return False
