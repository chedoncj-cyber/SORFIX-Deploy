"""
Hong Kong Exchanges and Clearing (HKEX) FIX 4.2 adapter.

Exchange details:
  Trading hours: 09:30–16:00 HKT (UTC+8)
  Settlement:    T+2
  Currency:      HKD
  FIX gateway:   fix.hkex.com.hk:9882 (TLS 1.3 required in production)
  Taker fee:     12 bps
"""
import logging
import threading
...
logger = logging.getLogger("hkex_adapter")

HKEX_CONFIG = VenueAdapterConfig(
    name="HKEX",
    host="fix.hkex.com.hk",
    port=9882,
    sender_comp_id="BROKER_HK",
    target_comp_id="HKEX_TRADING",
    currency="HKD",
    taker_fee=0.0012,
    latency_ms=8.0,
    enabled=True,
...
def hkex_tick_size(price: float) -> float:
...
def _round_to_tick(price: float) -> float:
    tick = hkex_tick_size(price)
    return round(round(price / tick) * tick, 6)


class HKEXAdapter(BaseExchangeAdapter):
    def __init__(self, config: VenueAdapterConfig = None):
        super().__init__(config or HKEX_CONFIG)
    ...
    def connect(self) -> bool:
        logger.info("HKEX: Connecting to %s:%s (TLS 1.3)", self.config.host, self.config.port)
        self._session.logged_on = True
        self._connected = True
        self._hb_stop.clear()
        self._hb_thread = threading.Thread(
            target=self._heartbeat_loop, name="hb-hkex", daemon=True
        )
        self._hb_thread.start()
        logger.info("HKEX: Session established")
        return True
    ...
    def _heartbeat_loop(self):
        ...
        while not self._hb_stop.wait(timeout=interval):
            try:
                ...
                logger.warning("HKEX: Heartbeat error: %s", exc)

    def subscribe_market_data(self, symbol: str) -> bool:
        if not self._connected:
            logger.warning("HKEX: Cannot subscribe %s — not connected", symbol)
            return False
        ...
        logger.info("HKEX: Subscribed market data for %s", symbol)
        return True

    def send_order(self, order_id: str, symbol: str, side: str, quantity: float, price: Optional[float]) -> bool:
        if not self._connected:
            logger.warning("HKEX: Cannot send order — not connected")
            return False
        ...
        logger.info("HKEX: Order [%s] %s %dx%s @ %s HKD", order_id, side.upper(), int(quantity), symbol, "MKT" if price is None else price)
        return True
    ...
    def get_status(self) -> dict:
        return {
            "venue": "HKEX", "connected": self._connected,
