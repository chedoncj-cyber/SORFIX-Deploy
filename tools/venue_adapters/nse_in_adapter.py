"""
National Stock Exchange of India (NSE_IN) FIX 4.2 adapter.

Exchange details:
  Trading hours: 09:15–15:30 IST (UTC+5:30) = 03:45–10:00 UTC
  Settlement:    T+1
  Currency:      INR (Indian Rupee)
  FIX gateway:   fix.nseindia.com:9887
  Taker fee:     3 bps
"""
import logging
import threading
...
logger = logging.getLogger("nse_in_adapter")

NSE_IN_CONFIG = VenueAdapterConfig(
    name="NSE_IN",
    host="fix.nseindia.com",
    port=9887,
    sender_comp_id="BROKER_IN",
    target_comp_id="NSE_IN_TRADING",
    currency="INR",
    taker_fee=0.0003,
    latency_ms=8.0,
    enabled=True,
...
# INR prices: small caps 5–500, large caps 100–5000, some 10000+
_TICK_TABLE = [
    (10.00,       0.05),
    (100.0,       0.25),
    (500.0,       0.50),
...
def nse_in_tick_size(price: float) -> float:
...
def _round_to_tick(price: float) -> float:
    tick = nse_in_tick_size(price)
    return round(round(price / tick) * tick, 6)


class NSEINAdapter(BaseExchangeAdapter):
    def __init__(self, config: VenueAdapterConfig = None):
        super().__init__(config or NSE_IN_CONFIG)
    ...
    def connect(self) -> bool:
        logger.info("NSE_IN: Connecting to %s:%s (TLS 1.3)", self.config.host, self.config.port)
        self._session.logged_on = True
        self._connected = True
        self._hb_stop.clear()
        self._hb_thread = threading.Thread(
            target=self._heartbeat_loop, name="hb-nse-in", daemon=True
        )
        self._hb_thread.start()
        logger.info("NSE_IN: Session established")
        return True
    ...
    def disconnect(self):
        ...
        self._connected = False
        logger.info("NSE_IN: Disconnected")
    ...
    def _heartbeat_loop(self):
        ...
        while not self._hb_stop.wait(timeout=interval):
            try:
                ...
                logger.warning("NSE_IN: Heartbeat error: %s", exc)

    def subscribe_market_data(self, symbol: str) -> bool:
        if not self._connected:
            logger.warning("NSE_IN: Cannot subscribe %s — not connected", symbol)
            return False
        ...
        logger.info("NSE_IN: Subscribed market data for %s", symbol)
        return True

    def send_order(self, order_id: str, symbol: str, side: str, quantity: float, price: Optional[float]) -> bool:
        if not self._connected:
            logger.warning("NSE_IN: Cannot send order — not connected")
            return False
        ...
        logger.info("NSE_IN: Order [%s] %s %dx%s @ %s INR", order_id, side.upper(), int(quantity), symbol, "MKT" if price is None else price)
        return True
    ...
    def get_status(self) -> dict:
        return {
            "venue": "NSE_IN", "connected": self._connected,
