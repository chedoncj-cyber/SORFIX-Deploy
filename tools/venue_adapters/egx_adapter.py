"""
Egyptian Exchange (EGX) FIX 4.2 adapter.

Exchange details:
  Trading hours: 10:00–14:30 EET (UTC+2)
  Settlement:    T+2
  Currency:      EGP (Egyptian Pound)
  FIX gateway:   fix.egx.com.eg:9881
  Taker fee:     15 bps
"""
import logging
import threading
...
logger = logging.getLogger("egx_adapter")

EGX_CONFIG = VenueAdapterConfig(
    name="EGX",
    host="fix.egx.com.eg",
    port=9881,
    sender_comp_id="BROKER_EG",
    target_comp_id="EGX_TRADING",
    currency="EGP",
    taker_fee=0.0015,
    latency_ms=6.0,
    enabled=True,
...
def egx_tick_size(price: float) -> float:
...
def _round_to_tick(price: float) -> float:
    tick = egx_tick_size(price)
    return round(round(price / tick) * tick, 6)


class EGXAdapter(BaseExchangeAdapter):
    def __init__(self, config: VenueAdapterConfig = None):
        super().__init__(config or EGX_CONFIG)
    ...
    def connect(self) -> bool:
        logger.info("EGX: Connecting to %s:%s", self.config.host, self.config.port)
        self._session.logged_on = True
        self._connected = True
        self._hb_stop.clear()
        self._hb_thread = threading.Thread(
            target=self._heartbeat_loop, name="hb-egx", daemon=True
        )
        self._hb_thread.start()
        logger.info("EGX: Session established")
        return True
    ...
    def _heartbeat_loop(self):
        ...
        while not self._hb_stop.wait(timeout=interval):
            try:
                ...
                logger.warning("EGX: Heartbeat error: %s", exc)

    def subscribe_market_data(self, symbol: str) -> bool:
        if not self._connected:
            logger.warning("EGX: Cannot subscribe %s — not connected", symbol)
            return False
        ...
        logger.info("EGX: Subscribed market data for %s", symbol)
        return True
    ...
    def send_order(
        ...
        if not self._connected:
            logger.warning("EGX: Cannot send order — not connected")
            return False
