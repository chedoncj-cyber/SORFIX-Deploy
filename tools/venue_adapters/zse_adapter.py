"""
Zimbabwe Stock Exchange (ZSE) FIX 4.2 adapter.

Exchange details:
  Trading hours: 08:30–15:00 CAT (UTC+2)
  Settlement:    T+3
  Currency:      ZWG (Zimbabwe Gold)
  FIX gateway:   fix.zse.co.zw:9889
  Taker fee:     20 bps
"""
import logging
import threading
...
logger = logging.getLogger("zse_adapter")

ZSE_CONFIG = VenueAdapterConfig(
    name="ZSE",
    host="fix.zse.co.zw",
    port=9889,
    sender_comp_id="BROKER_ZW",
    target_comp_id="ZSE_TRADING",
    currency="ZWG",
    taker_fee=0.0020,
    latency_ms=25.0,
    enabled=True,
...
def zse_tick_size(price: float) -> float:
...
def _round_to_tick(price: float) -> float:
    tick = zse_tick_size(price)
    return round(round(price / tick) * tick, 6)


class ZSEAdapter(BaseExchangeAdapter):
    def __init__(self, config: VenueAdapterConfig = None):
        super().__init__(config or ZSE_CONFIG)
    ...
    def connect(self) -> bool:
        logger.info("ZSE: Connecting to %s:%s", self.config.host, self.config.port)
        self._session.logged_on = True
        self._connected = True
        self._hb_stop.clear()
        self._hb_thread = threading.Thread(
            target=self._heartbeat_loop, name="hb-zse", daemon=True
        )
        self._hb_thread.start()
        logger.info("ZSE: Session established")
        return True
    ...
    def _heartbeat_loop(self):
        ...
        while not self._hb_stop.wait(timeout=interval):
            try:
                ...
                logger.warning("ZSE: Heartbeat error: %s", exc)

    def subscribe_market_data(self, symbol: str) -> bool:
        if not self._connected:
            logger.warning("ZSE: Cannot subscribe %s — not connected", symbol)
            return False
        ...
        logger.info("ZSE: Subscribed market data for %s", symbol)
        return True
    ...
    def send_order(
        ...
        if not self._connected:
            logger.warning("ZSE: Cannot send order — not connected")
            return False
