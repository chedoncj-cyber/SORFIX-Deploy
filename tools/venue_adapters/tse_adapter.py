"""
Tokyo Stock Exchange (TSE) FIX 4.2 adapter.

Exchange details:
  Trading hours: 09:00–15:30 JST (UTC+9)
  Settlement:    T+2
  Currency:      JPY
  FIX gateway:   fix.jpx.co.jp:9883 (TLS 1.3 required in production)
  Taker fee:     8 bps
"""
import logging
import threading
...
logger = logging.getLogger("tse_adapter")

TSE_CONFIG = VenueAdapterConfig(
    name="TSE",
    host="fix.jpx.co.jp",
    port=9883,
    sender_comp_id="BROKER_JP",
    target_comp_id="TSE_TRADING",
    currency="JPY",
    taker_fee=0.0008,
    latency_ms=10.0,
    enabled=True,
...
class TSEAdapter(BaseExchangeAdapter):
    def __init__(self, config: VenueAdapterConfig = None):
        super().__init__(config or TSE_CONFIG)
    ...
    def connect(self) -> bool:
        logger.info("TSE: Connecting to %s:%s (TLS 1.3)", self.config.host, self.config.port)
        self._session.logged_on = True
        self._connected = True
        self._hb_stop.clear()
        self._hb_thread = threading.Thread(
            target=self._heartbeat_loop, name="hb-tse", daemon=True
        )
        self._hb_thread.start()
        logger.info("TSE: Session established")
        return True
    ...
    def _heartbeat_loop(self):
        ...
        while not self._hb_stop.wait(timeout=interval):
            try:
                ...
                logger.warning("TSE: Heartbeat error: %s", exc)

    def subscribe_market_data(self, symbol: str) -> bool:
        if not self._connected:
            logger.warning("TSE: Cannot subscribe %s — not connected", symbol)
            return False
        ...
        logger.info("TSE: Subscribed market data for %s", symbol)
        return True

    def send_order(self, order_id: str, symbol: str, side: str, quantity: float, price: Optional[float]) -> bool:
        if not self._connected:
            logger.warning("TSE: Cannot send order — not connected")
            return False
        ...
        logger.info("TSE: Order [%s] %s %dx%s @ %s JPY", order_id, side.upper(), int(quantity), symbol, "MKT" if price is None else price)
        return True
    ...
    def get_status(self) -> dict:
        return {
            "venue": "TSE", "connected": self._connected,
