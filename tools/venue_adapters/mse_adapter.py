"""
Malawi Stock Exchange (MSE) FIX 4.2 adapter.

Exchange details:
  Trading hours: 07:30–13:30 CAT (UTC+2)
  Settlement:    T+3
  Currency:      MWK (Malawian Kwacha)
  FIX gateway:   fix.mse.co.mw:9886
  Taker fee:     20 bps
"""
import logging
import threading
...
logger = logging.getLogger("mse_adapter")

MSE_CONFIG = VenueAdapterConfig(
    name="MSE",
    host="fix.mse.co.mw",
    port=9886,
    sender_comp_id="BROKER_MW",
    target_comp_id="MSE_TRADING",
    currency="MWK",
    taker_fee=0.0020,
    latency_ms=20.0,
    enabled=True,
...
# MWK is a high-nominal currency; prices typically 1–2000 MWK
_TICK_TABLE = [
    (10.0,        0.1),
    (100.0,       0.5),
    (500.0,       1.0),
...
def mse_tick_size(price: float) -> float:
...
def _round_to_tick(price: float) -> float:
    tick = mse_tick_size(price)
    return round(round(price / tick) * tick, 6)


class MSEAdapter(BaseExchangeAdapter):
    def __init__(self, config: VenueAdapterConfig = None):
        super().__init__(config or MSE_CONFIG)
    ...
    def connect(self) -> bool:
        logger.info("MSE: Connecting to %s:%s", self.config.host, self.config.port)
        self._session.logged_on = True
        self._connected = True
        self._hb_stop.clear()
        self._hb_thread = threading.Thread(
            target=self._heartbeat_loop, name="hb-mse", daemon=True
        )
        self._hb_thread.start()
        logger.info("MSE: Session established")
        return True
    ...
    def _heartbeat_loop(self):
        ...
        while not self._hb_stop.wait(timeout=interval):
            try:
                ...
                logger.warning("MSE: Heartbeat error: %s", exc)

    def subscribe_market_data(self, symbol: str) -> bool:
        if not self._connected:
            logger.warning("MSE: Cannot subscribe %s — not connected", symbol)
            return False
        ...
        logger.info("MSE: Subscribed market data for %s", symbol)
        return True
    ...
    def send_order(
        ...
        if not self._connected:
            logger.warning("MSE: Cannot send order — not connected")
            return False
